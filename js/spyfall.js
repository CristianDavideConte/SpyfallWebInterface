...
function init() {
	...
	createGameTable();											//The gameTableElement is filled with the necessary game inital components
	...
	setTimeout(() => showRules(), 600);							//After all the animations have completed the game's rules are shown to the user
}

/* This function creates a webWorker which will parse the files containig the game's roles and scenaries.
 * The worker will evenctually post a message error if the fetch of those files failed.
*/
function parsingWorkerSetup() {
	let roles;
	let scenaries;
	parsingWorker = new Worker("./js/languageDependencesParser.js?lan=it");		//Creates the worker
	parsingWorker.addEventListener("message", event => {
		let data = event.data;

		if(Array.isArray(data)) {
			let length = data.length;
			if(data[length - 1] == "roles") {
				roles = data;
				roles.splice(length - 1);						//Removes the identifier "roles" present in the passed data but not necessary anymore
			} else if(data[length - 1] == "scenaries") {
				scenaries = data;
				scenaries.splice(length - 1);					//Removes the identifier "scenaries" present in the passed data but not necessary anymore
			}
		}
		console.log("Roles",roles);
		console.log("Scenaries",scenaries);

		if(roles != null && scenaries != null)
...
function createGameTable() {
	...
	gameTableButtonSectionElement.setAttribute("class", "menuButtonSection");

	if(gameElement.childElementCount > 1) {																		//Everything but the gameTable's title is deleted
		gameElement.removeChild(gameTableElement);
		gameElement.removeChild(gameElement.lastElementChild);
	}
...
function showPopUpMenu(titleSection, content, buttonSection, menuEventsMap) {
	window.requestAnimationFrame(() => {
		...
		});																									//Makes the menu animate (slides from bottom)
	});
}
...
function removeMenuAndRestoreMainPage() {
	window.requestAnimationFrame(() => {
		...
		let _menuButtonSection = menuElement.lastChild;
		for(const _menuButton of _menuButtonSection.childNodes)
			_menuButton.disabled = true;							//Prevents the user to trigger this function twice by clicking the menu buttons again
		...
		setTimeout(() => menuSection.innerHTML = "", 300);														//Remove the currently displayed popUpMenu from HTML menu div after it has animated
...
/*------------------------TEST SECTION ---------------------------*/
var test = 0;
function popUpMenuTest() {
	if(test % 2 == 0)
		showScenaryNotChosenMenu();
	else {
			var _testEvent = document.createEvent('Events');
		  _testEvent.initEvent("click", true, false);
			menuOverlayElement.dispatchEvent(_testEvent);
	}

	test++;
	if(test < 50)
		setTimeout(popUpMenuTest, 400);
}
